	...
	if !IsValid(GAMEMODE.Vars.Monitor) then
		...
		if control then
			...
			for k, v in pairs(GAMEMODE.Vars.Animatronics) do
				...
				GAMEMODE.Vars.Monitor.Animatronics[k].Txt:SetFont("FNAFGMNIGHT")
		...
		if !nope then
			...
			if game.GetMap() == "freddysnoevent" then
				...
				if !control then
					...
					GAMEMODE.Vars.Monitor.KitchenText2:SetPos(ScrW() / 2 - 150, 140)
			...
			elseif game.GetMap() == "fnaf2noevents" then
				...
				if !control then
					...
					CloseT:SetPos(ScrW() / 2 + 32, ScrH() - 130)
					CloseT:SetText("")
					CloseT.DoClick = function(button)
						if IsValid(FNaFView) then waitt = CurTime() + 1 end
						GAMEMODE.Vars.Monitor:Close()
		...
		function GAMEMODE.Vars.Monitor:MoveMenu(apos)
			...
			for k, v in pairs(GAMEMODE.Vars.Animatronics) do
				...
				if GAMEMODE.Vars.Animatronics[k][2] != apos and GAMEMODE.Vars.Animatronics[k][2] != GAMEMODE.APos[game.GetMap()].Office
				and GAMEMODE.AnimatronicAPos[k] and GAMEMODE.AnimatronicAPos[k][game.GetMap()] and GAMEMODE.AnimatronicAPos[k][game.GetMap()][apos]
				and GAMEMODE.Vars.Animatronics[k][3] != -1 and SysTime() + GAMEMODE.Vars.Animatronics[k][3] <= SysTime() then
