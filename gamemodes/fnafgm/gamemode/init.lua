...
function GM:PlayerSpawn(ply)
	...
	if fnafgmPlayerCanByPass(ply,"jump") then
	...
	if GAMEMODE.Vars.b87 then
		...
		timer.Create("fnafgm87" .. userid, 20, 1, function()
			if IsValid(ply) and (!ply:IsListenServerHost() and !fnafgmPlayerCanByPass(ply, "debug")) then
				ply:Kick("'87")
			end
	...
	if fnafgm_timethink_autostart:GetBool() and !fnafgm_sandbox_enable:GetBool() and !timer.Exists("fnafgmStart") and !GAMEMODE.Vars.startday and !GAMEMODE.Vars.tempostart and GAMEMODE.MapList[game.GetMap()] then
		...
		if !game.SinglePlayer() then
			...
			timer.Create("fnafgmStart", 0.02, 1, function()
					...
					if (!GAMEMODE.Vars.nightpassed and IsValid(ply) and !ply:Alive()) then
						...
						if !game.SinglePlayer() then
							...
							if fnafgm_respawndelay:GetInt() == 0 then
								...
								if ((fnafgm_autorespawn:GetBool() and fnafgm_respawnenabled:GetBool()) and !GAMEMODE.Vars.norespawn) then
									ply:Spawn()
								...
								if fnafgm_respawnenabled:GetBool() and !GAMEMODE.Vars.norespawn then
...
function fnafgmUse(ply, ent, test, test2)
	...
	if game.GetMap() == "freddysnoevent" then
		...
		if door1btn and IsValid(door1btn) and ent == door1btn and !GAMEMODE.Vars.DoorWait1 then
		...
		if door2btn and IsValid(door2btn) and ent == door2btn and !GAMEMODE.Vars.DoorWait2 then
	...
	if game.GetMap() == "fnaf2noevents" then
		...
		if light3 and IsValid(light3) and ent == light3 then
			...
			if !GAMEMODE.Vars.light3usewait and !GAMEMODE.Vars.poweroff then
				...
				if !GAMEMODE.Vars.LightUse[3] then light3:Fire("use") end
...
function GM:Think()
	...
	if !GAMEMODE.Vars.checkRestartNight and GAMEMODE.Vars.iniok and GAMEMODE.Vars.mapoverrideok and GAMEMODE.Vars.startday and GAMEMODE.Vars.active then
